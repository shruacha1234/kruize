...
KRUIZE_DOCKER_IMAGE="kruize/kruize:latest"
...
function docker_prereq() {
	...
	docker pull ${CADVISOR_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	err_exit "Error: Unable to pull cadvisor docker image: ${CADVISOR_DOCKER_IMAGE}"

	docker pull ${PROMETHEUS_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	err_exit "Error: Unable to pull prometheus docker image: ${PROMETHEUS_DOCKER_IMAGE}"

	

	docker pull ${KRUIZE_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	if [ $? != 0 ]; then
...
function docker_setup() {
	...
	docker run -d --rm --name=cadvisor   -p ${CADVISOR_PORT}:${CADVISOR_PORT}      --net=${NETWORK}   --cpus=1   --volume=/:/rootfs:ro  --volume=/var/run:/var/run:ro   --volume=/sys:/sys:ro   --volume=/var/lib/docker/:/var/lib/docker:ro   --volume=/dev/disk/:/dev/disk:ro  ${CADVISOR_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	err_exit "Error: cadvisor did not start up"

	echo "Starting prometheus container...."
	docker run -d --rm --name=prometheus -p ${PROMETHEUS_PORT}:${PROMETHEUS_PORT}  --net=${NETWORK} -v ${ROOT_DIR}/${PROMETHEUS_MANIFEST}:/etc/prometheus/prometheus.yml ${PROMETHEUS_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	err_exit "Error: prometheus did not start up"
	echo "done"
...
function docker_deploy() {
	...
	echo "Info: Waiting for prometheus/cadvisor to be up and running.."
	sleep 2
	echo "Starting kruize container...."
	docker run -d --rm --name=kruize     -p ${KRUIZE_PORT}:${KRUIZE_PORT}          --net=${NETWORK} --env CLUSTER_TYPE="DOCKER" --env MONITORING_AGENT_ENDPOINT="http://prometheus:${PROMETHEUS_PORT}" --env MONITORING_AGENT="Prometheus" -v ${ROOT_DIR}/${DOCKER_JSON}:/opt/app/kruize-docker.json ${KRUIZE_DOCKER_IMAGE} 2>>${LOGFILE} >>${LOGFILE}
	err_exit "Error: kruize did not start up"
...
echo -n "             Installing kruize for docker..."
	docker_start
echo "done"
